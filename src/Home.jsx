...
let IntroA = memo( function IntroA( { sliding } ) {
  let [ intro, setIntro ] = useState( 0 )
...
let DynCount = memo( function DynCount() {
  ...
  if ( isIntersecting ) {
    if ( ind == 0 ) {
      ...
      let widtF = 1;
      setind( 1 );
      let idA = setInterval( frameA, 10 );
      let idB = setInterval( frameB, 10 );
      let idC = setInterval( frameC, 10 );
      let idD = setInterval( frameD, 10 );
      let idE = setInterval( frameE, 10 );
      let idF = setInterval( frameF, 100 );
    }
    function frameA() {
      if ( widtA >= 600 ) {
        clearInterval( idA );
...
let Filter = memo( function Filter() {
...
let Clients = memo( function Clients() {
  let [ cli, setCli ] = useState( 1 )
  let [ cliv, setCliv ] = useState( 0 )
